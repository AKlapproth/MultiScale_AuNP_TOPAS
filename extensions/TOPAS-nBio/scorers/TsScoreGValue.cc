...
TsScoreGValue::TsScoreGValue(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsScoringManager* scM, TsExtensionManager* eM,
...
TsScoreGValue::~TsScoreGValue()
{;}


G4bool TsScoreGValue::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
	if (!fIsActive) {
...
void TsScoreGValue::AccumulateEvent() {
...
void TsScoreGValue::AbsorbResultsFromWorkerScorer(TsVScorer* workerScorer) {
	TsVNtupleScorer::AbsorbResultsFromWorkerScorer(workerScorer);
	
	TsScoreGValue* workerGvalueScorer = dynamic_cast<TsScoreGValue*>(workerScorer);
