...
// ********************************************************************
//
// $Id: TsDNAOneStepThermalizationModel.cc 101807 2016-11-30 13:42:28Z gunter $
//
...
namespace DNA{ namespace Penetration{
    ...
    const double
    Terrisol1990A::gEnergies_T1990[11] =
    { 0.2, 0.5, 1, 2, 3, 4, 5, 6, 7,
        // The two last are not in the dataset
        8, 9}; // eV
    
    const double
    Terrisol1990A::gStdDev_T1990[11] =
    ...
    double Meesungnoen2002A::GetRmean(double k){
        G4double k_eV = k/eV;
    ...
    void Meesungnoen2002A::GetPenetration(G4double k,
    ...
    double Terrisol1990A::Get3DStdDeviation(double energy){
    ...
    double Terrisol1990A::GetRmean(double energy){
    ...
    void Terrisol1990A::GetPenetration(G4double energy,
                                       G4ThreeVector& displacement){
        double sigma3D=Get3DStdDeviation(energy);
    ...
    //----------------------------------------------------------
    double Ballarini::GetRmean(double k) {
        G4double k_eV = k/eV;
        if ( k_eV > 0.1 )
            return 1.8 * k_eV * CLHEP::nanometer;
        return 0;
    }
    
    void Ballarini::GetPenetration(G4double k,
                                   G4ThreeVector& displacement) {
        displacement=G4ThreeVector(0,0,0);
        G4double k_eV = k/eV;
        if ( k_eV > 0.1) {
            G4double r_mean = 1.8 * k_eV * CLHEP::nanometer;
